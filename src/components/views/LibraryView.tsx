...
export function LibraryView() {
  ...
  return (
    <div className="space-y-6 pb-6 text-white select-none animate-in fade-in duration-200">
      ...
      {/* Apple Music Style Storage & Sandbox Widget */}
      <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-emerald-500/[0.05] to-transparent border border-emerald-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-bold text-white">Private Offline Storage</h3>
              <span className="text-[9px] font-mono font-extrabold px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
                {downloadedSongIds.length} Saved
              </span>
            </div>
            <p className="text-xs text-[#8E92A4] mt-0.5">
              {formatBytes(storageInfo?.raagaXUsed || 0)} used of {formatBytes(storageInfo?.quota || 64 * 1024 * 1024 * 1024)} sandboxed quota
            </p>
          </div>
        </div>
        ...
        <button
          onClick={() => {
            setTab('downloads');
            setDownloadSubTab('storage');
          }}
          className="px-4 py-2 rounded-xl text-xs font-bold bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 transition-all flex items-center gap-1.5 cursor-pointer ml-auto sm:ml-0"
        >
          Manage Storage <ChevronRight className="w-3.5 h-3.5" />
        </button>
      </div>
    </div>
  );
