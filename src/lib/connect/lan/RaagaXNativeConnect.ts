...
import { LANDeviceAdvertisement, LANMessage } from './types';
...
export class RaagaXNativeConnect {
  ...
  public static setIncomingMessageListener(listener: (msg: LANMessage) => void) {
    ...
    try {
      const cap = (window as any).Capacitor;
      if (cap?.Plugins?.RaagaXConnectPlugin?.addListener) {
        cap.Plugins.RaagaXConnectPlugin.addListener('onDirectMessageReceived', (msg: LANMessage) => {
          this.messageListener?.(msg);
        });
      }
    } catch (e) {
      console.warn('[RaagaXNativeConnect] setIncomingMessageListener error:', e);
    }
  }

  public static async sendDirectMessage(targetHost: string, port: number, message: LANMessage): Promise<boolean> {
    if (!this.isNative()) return false;

    try {
      const cap = (window as any).Capacitor;
      if (cap?.Plugins?.RaagaXConnectPlugin?.sendMessage) {
        const res = await cap.Plugins.RaagaXConnectPlugin.sendMessage({
          host: targetHost,
          port,
      ...
      console.warn('[RaagaXNativeConnect] sendDirectMessage error:', e);
    }
    return false;
  }
}
