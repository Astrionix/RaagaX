export const WEEKLY_RELEASE_SOURCES: Record<string, { id: string; title: string }> = {
  Telugu: {
    id: "37i9dQZF1DWWwrjLPC16W7",
    title: "Latest Telugu",
  },
  Tamil: {
    id: "37i9dQZF1DWVo4cdnikh7Z",
    title: "Latest Tamil",
  },
  Kannada: {
  ...
  Hindi: {
    id: "37i9dQZF1DX4ghkRUdIogy",
    title: "New Music Friday India",
  },
  Malayalam: {
    id: "4d7KoENUtm2oyb651XftMg",
...
export const NEW_RELEASES_SOURCES = WEEKLY_RELEASE_SOURCES;
export const HOT_HITS_SOURCES = WEEKLY_RELEASE_SOURCES;
...
export const CATEGORY_SPOTIFY_SOURCES: Record<string, Record<string, { id: string; title: string }>> = Object.fromEntries(
  Object.entries(BROWSE_5_PLAYLISTS).map(([lang, items]) => [
    lang,
    {
      ...
      new_music: WEEKLY_RELEASE_SOURCES[lang] || { id: '37i9dQZF1DWWwrjLPC16W7', title: `This Week's ${lang} Releases` },
      charts: { id: TRENDING_SOURCES[lang]?.id || items[0]?.id || '37i9dQZF1DWTt3gMo0DLxA', title: TRENDING_SOURCES[lang]?.title || `${lang} Charts` },
      ...
      genres: { id: CLASSICS_SOURCES[lang]?.id || items[4]?.id || '37i9dQZF1DWWwrjLPC16W7', title: CLASSICS_SOURCES[lang]?.title || `${lang} Classics` }
    }
  ])
);
